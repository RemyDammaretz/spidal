...
static void setupGUI(GtkApplication * app, gpointer user_data) {
    ...
    shutDownButton = GTK_BUTTON(gtk_builder_get_object(builder, "shutDownButton"));
    shutDownImage = GTK_IMAGE(gtk_builder_get_object(builder, "shutDownButtonImage"));
    pixbufShutdownImage = gdk_pixbuf_new_from_file_at_scale(IMG_SHUTDOWN, 20, 20, TRUE, NULL);
    gtk_image_set_from_pixbuf(shutDownImage, pixbufShutdownImage);
...
void * applicationThread() {
    ...
    //ppid = getpid();
    CHECK(pipe(pipefd), "pipe()"); // Communication between procesus with pipe
    CHECK(spidalBackPid = fork(), "fork()");
    if (spidalBackPid == 0) {
        // Child process : spidalBack

        // // Kill child process when parent dies in order to close all tcp port used
        // CHECK(prctl(PR_SET_PDEATHSIG, SIGTERM), "prctl()");
        // // Test in case parent process exited just before prctl call
        // if (getppid() != ppid) exit(1);
        
        CHECK(close(pipefd[0]), "close()"); // Close reading end in the child

        CHECK(dup2(pipefd[1], 1), "dup2()"); // Send stdout to the pipe
        //CHECK(dup2(pipefd[1], 2), "dup2()"); // Send stderr to the pipe

        CHECK(close(pipefd[1]), "close()"); // Descriptor no longer needed

        CHECK(execl(SPIDAL_BACK_PATH, "spidal", NULL), "execl()");
    } else {
        // Parent process
        CHECK(close(pipefd[1]), "close()"); // Close writting end in the parent

        // Read data
        while (read(pipefd[0], &byte, 1) == 1) {
            if (ioctl(pipefd[0], FIONREAD, &count) != -1) {
                data = malloc(count + 1);
                data[0] = byte;
                if (read(pipefd[0], data + 1, count) == count) {
                    // Use data
...
void startRace() {
    ...
    if (getStatus(&runnerStatus[0]) == RUNNER_READY && getStatus(&runnerStatus[1]) == RUNNER_READY) {
        ...
        do {
            usleep(100);
            clock_gettime(CLOCK_REALTIME, &time);
        } while (timeCompare(&time.tv_sec, &time.tv_nsec, &timeRaceStart.tv_sec, &timeRaceStart.tv_nsec) == -1);
